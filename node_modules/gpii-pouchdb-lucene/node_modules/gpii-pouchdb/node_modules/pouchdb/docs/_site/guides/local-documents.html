...
<html lang="en" manifest="/manifest.appcache">
  ...
  <div class="container">
    ...
    <a href="/guides/local-documents.html">Local documents</a>
        ...
        <div class="col-sm-9">
          <p>&quot;Local&quot; documents are a special class of documents in PouchDB and CouchDB, which are used for storing local metadata about a database. You might never need them in your own app, but sometimes they can come in handy for advanced use cases.</p>

<h2>Local docs in a nutshell</h2>

<p>Local docs have the following characteristics:</p>

<ul>
<li>They don&#39;t replicate.</li>
<li>They can&#39;t contain attachments.</li>
<li>They don&#39;t appear in <code>allDocs()</code>, <code>changes()</code>, or <code>query()</code>.</li>
<li>However, you can modify them with <code>put()</code>/<code>remove()</code>/<code>bulkDocs()</code>, and you can fetch them with <code>get()</code>.</li>
</ul>

<p>So basically, local docs only exist <em>for that database</em>, and they don&#39;t mix with the &quot;normal&quot; documents.</p>

<p>To create a local doc, you simply use <code>&#39;_local/&#39;</code> as the prefix of the <code>_id</code>. This is supported in both CouchDB and PouchDB:</p>
<div class="highlight"><pre><code class="language-js" data-lang="js"><span class="nx">db</span><span class="p">.</span><span class="nx">put</span><span class="p">({</span>
  <span class="nx">_id</span><span class="o">:</span> <span class="s1">&#39;_local/foobar&#39;</span><span class="p">,</span>
  <span class="nx">someText</span><span class="o">:</span> <span class="s1">&#39;yo, this is my local doc!&#39;</span>
<span class="p">}).</span><span class="nx">then</span><span class="p">(</span><span class="kd">function</span> <span class="p">()</span> <span class="p">{</span>
  <span class="k">return</span> <span class="nx">db</span><span class="p">.</span><span class="nx">get</span><span class="p">(</span><span class="s1">&#39;_local/foobar&#39;</span><span class="p">);</span>
<span class="p">});</span>
</code></pre></div>
<h2>Advantages of local docs</h2>

<p>Local docs are useful for small bits of configuration or metadata, which you don&#39;t necessarily want to replicate, but which you want to keep in the database anyway. Many PouchDB plugins and core components use local docs. For instance, the replication algorithm uses them to store checkpoints, and map/reduce uses them to keep track of what&#39;s been <code>emit</code>ted.</p>

<p>Local docs also have some good performance characteristics compared to regular docs. They don&#39;t have a version history, so only the most recent revision is ever stored in the database. This means that <code>put()</code>s and <code>get()</code>s are faster for local docs than for regular docs, and that local docs tend to take up less space on disk.  In a sense, they are auto-compacted, although they take up even less space on disk than documents in a compacted database.</p>

<p>Regardless, you need to provide the current <code>_rev</code> when you update local docs, just like with regular docs.</p>
...
<li><a href="/api.html#delete_document">remove()</a></li>
        ...
        <ul class="pager">
                ...
                <li class="previous">
                    <a href="/guides/compact-and-destroy.html">&larr; Compacting and destroying</a>
                </li>
