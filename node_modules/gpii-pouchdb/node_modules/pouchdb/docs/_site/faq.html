...
<p>In <strong>Firefox</strong>, PouchDB uses IndexedDB. Though Firefox has no upper limit besides disk space, if your application wishes to store more than 50MB locally, Firefox will <a href="https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API">ask the user</a> to confirm that this is okay.</p>

<p><strong>Chrome</strong> also uses IndexedDB, it determines the amount of storage left available on the user&#8217;s hard drive and uses <a href="https://developers.google.com/chrome/whitepapers/storage#temporary">that to calculate a limit</a>.</p>

<p><strong>Opera 15+</strong> shares a codebase Chromium / Blink, and behaves similarly.</p>

<p><strong>Internet Exporer 10+</strong> has a hard 250MB limit.</p>

<p><strong>Mobile Safari</strong> on iOS has a hard 50MB limit, while <strong>desktop Safari &le;7</strong> will prompt the user if an application requests more than 5MB of data, up to a limit of 500MB. Some versions of Safari will only let you request additional storage once, but you can get around this using <a href="http://pouchdb.com/api.html#create_database">the <code>size</code> option</a>.</p>
