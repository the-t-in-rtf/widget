			...
			if(comment.replies.length > 0){
				...
				$.each(comment.replies, function(){
					...
					if(WidgetConf.isVendor && this.user == WidgetConf.user){
						var reply_edit   = $('<a>').attr({href:'#'}).text('Edit reply').addClass('edit_reply').data({IdReply: this.id, IdComment: comment.id});
						var reply_delete = $('<a>').attr({href:'#'}).text('Delete reply').addClass('delete_reply').data({IdReply: this.id, IdComment: comment.id});
